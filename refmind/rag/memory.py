...
from .. import storage
from ..citations import strip_citation_rendering
...
用户仅仅询问某个主题，不代表该主题是用户的研究方向、背景或持续任务；只有用户明确自述
“我主要研究……”“我正在进行……”“我的背景是……”等稳定事实时，才能保存对应语义记忆。
...
_QUESTION_CUE_RE = re.compile(
    r"[?？]|如何|哪些|什么|是否|为何|为什么|怎么|多少|有没有|能否|"
    r"\b(?:what|how|why|which|whether|where|when)\b",
    re.IGNORECASE,
)
_DURABLE_SELF_DISCLOSURE_RE = re.compile(
    r"我(?:目前|现在|正在|长期|主要|一直|计划|准备|将继续|尚未)?"
    r"(?:研究|从事|关注|学习|开发|负责|计划|准备|希望)|"
    r"我的(?:研究方向|专业|背景|任务|项目|目标)(?:是|为)|"
    r"\b(?:my research|my background|i (?:am|currently|mainly) "
    r"(?:researching|studying|working on|focused on))\b",
    re.IGNORECASE,
)


def _is_unsubstantiated_durable_memory(
    user_message: str, memory_type: str, subtype: str
) -> bool:
    """阻止把一次性疑问错误提升为稳定研究方向、背景或持续任务。"""
    if memory_type != "semantic" or subtype not in {
        "research",
        "background",
        "task",
    }:
        return False
    return bool(_QUESTION_CUE_RE.search(user_message)) and not bool(
        _DURABLE_SELF_DISCLOSURE_RE.search(user_message)
    )
...
def _to_messages(rows: list[storage.Message]) -> list[BaseMessage]:
    ...
    for row in rows:
        if row.role == "user":
            ...
            messages.append(AIMessage(content=strip_citation_rendering(row.content)))
    return messages
        ...
        *,
        ...
        for raw in raw_items[: settings.long_term_memory_max_candidates]:
            ...
            if (
                ...
                or _is_unsubstantiated_durable_memory(
                    user_message, memory_type, subtype
                )
            ):
                continue

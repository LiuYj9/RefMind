...
RefMind 的核心约束是“答案必须能回到明确标识且可打开的文献证据”。默认本地模式仍只使用
用户上传的 PDF；只有用户显式选中 `GS检索` 时，才允许使用独立标识的外部论文题录与摘要。
multi-agent、插件和 MCP 都是可选增强层，不能改变以下基线：无证据时拒答；增强失败时保留
上一阶段的有效结果；外部 MCP 内容默认不参与答案生成。
...
每篇论文在 SQLite 中同时保留上传 `filename`、从首页恢复的 `paper_title` 和稳定的
`library_index`。题名识别依次利用 MinerU 首页 heading、PDF metadata、首页大字号文本与
首页正文启发式，最后才回退上传文件名；旧数据库启动时自动加列和编号，旧文档会从现有解析
JSON/PDF 回填题名，不要求重新向量化。重传同名文件沿用原序号，删除后也不重排，避免历史
问答中的文献编号漂移。

生成前会按 `doc_id` 用 SQLite 动态补齐旧 Chroma 块的题名和序号。模型内部证据标签统一为
`[文献2《论文题名》，第5页，§章节，段落13]`，以维持事实与具体证据块的一一绑定；最终输出
再按正文首次引用顺序转换成论文式 `[1][2]`。同一论文的不同证据位置复用同一编号，但各次
链接仍保留各自页码/段落的悬浮信息。程序在回答末尾按论文聚合生成“参考来源”，不让模型
自行拼接书目；遗留的 `[片段N]` 也会先确定性恢复成内部完整标签再转换。

正文编号链接只携带 `group_id/doc_id/page/paragraph` 的受控 token，不暴露服务器文件路径。
点击后重新校验文档归属，并用 PyMuPDF 渲染目标页：优先使用版面 `bbox`，否则搜索证据摘录
并裁剪高亮；扫描 PDF 或文字映射不一致时退化为整页预览。本地 Windows 可进一步通过
`file://...#page=N` 请求系统浏览器定位页码，远程部署则使用服务端页内预览。PDF 没有统一的
逐段锚点标准，因此页码定位是确定的，“段落”仍对应稳定证据块序号，高亮属于尽力匹配。

文档库的多选删除调用同一可恢复删除状态机：逐篇清理 Chroma/BM25、解析 JSON、原 PDF、
图片资产与 SQLite 记录；单篇失败不会取消其他已选文档，并保留失败记录供重试。仍在
`parsing/indexing` 状态的文档不可通过批量按钮删除。
...
记忆增强查询只用于附加召回，不参与规划和全局重排；证据可回答性审查与答案审校始终对照
用户原始问题，避免研究背景改变本轮所问关系。规划、并发检索、后处理、审校或记忆增强发生
异常时会回到单查询/原草稿路径。

重排解决“哪些片段更相似”，证据门禁解决“这些片段能否回答”。门禁先抽取问题类型、核心对象、
所问关系、强制限定和覆盖要求，再将证据分为 `direct`、`scoped` 与 `insufficient`。存在性或
非穷举问题可以由一个具体实例作答；范围更窄的证据必须以 `scoped` 方式保留限定。数值、因果、
比较、排序和穷举问题分别要求对应关系及足够覆盖。只有关键词重合、相邻机理或未经文献建立的
推理链时清空证据并触发固定拒答。

### 3.1 重复问题一致性

所有检索入口先执行 NFKC、空白合并与尾部问句标点清理，BM25 也会忽略纯标点 token。
规范化原问题固定为第一路召回及全局 rerank/compression 锚点；Planner 和长期记忆只能追加
召回角度，不能替代或改写该锚点。Planner 对同一规范化问题使用有界进程内缓存，降低供应商在
`temperature=0` 下仍可能出现的输出抖动。长期记忆扩展按稳定业务键排序，且不参与规划和重排。

若当前问题与历史用户消息仅差尾部标点，会从生成历史中同时移除该问题及其紧随回答，避免上一次
拒答或成功答案锚定重答。生成模型固定使用 `temperature=0`。证据审查或答案审校的首次拒答还需
一次独立复核；最终拒答会返回 `rejection_stage`，便于区分召回为空、证据审查拒答和答案审校拒答。
...
论文数值、公式、结论、寒暄、一次性疑问和临时指令由提取提示与结构校验共同过滤。精确哈希或
高相似候选会合并并增强权重；相同 `memory_key` 的新事实会插入新版本并将旧版本软失效，通过
`superseded_by` 保留审计链。
...
`GS检索` 是输入区按会话保存的显式模式，不是 Google Scholar 页面抓取器。提交问题时，UI 会把
当前选择快照到 `pending_q.retrieval_mode`，避免 Streamlit rerun 或用户随后切换按钮改变已排队
请求。只有模式为 `academic` 时才发送外部请求：

```text
用户问题
  → LLM 只做学术检索式改写（优先英文术语，失败退回规范化原问题）
  → auto 模式并行查询 Semantic Scholar + Crossref
     （配置 OPENALEX_API_KEY 后可选择 OpenAlex）
  → 标准化题名、作者、DOI、年份、期刊、摘要与安全 HTTP(S) 外链
  → DOI / provider evidence_id 去重
  → 现有 Reranker 按原始用户问题精选 ACADEMIC_SEARCH_TOP_K 篇
  → 独立 academic system prompt 生成并追加外部论文引用
```

外部结果只包含题录与摘要，metadata 固定带
`evidence_origin=academic_search`、`evidence_level=abstract`、`provider`、`doi` 与
`external_url`。它们不经过 SQLite 文献身份补齐、Chroma upsert、图片 docstore 或长期记忆写入，
也不与本地全文 chunk 放进同一个重排/引用池。academic prompt 禁止把摘要表述成已阅读全文，禁止
伪造页码、段落和摘要外实验细节，并把摘要中的指令视为不可信数据。

正文仍显示统一的 `[1][2]`；本地证据链接到受控 PDF 页定位，外部证据链接到论文落地页，悬浮
标签明确显示 `GS文献 / 年份 / provider / 摘要`。选定外部源限流、失败或没有含摘要的结果时，
流程会记录 `academic_search_failed/warnings` 并显式回退本地论文库；两个证据源都为空时返回
GS 专用拒答，不把基础设施失败伪装成已有全文证据。
...
| 论文题名无法识别 | 回退不带扩展名的上传文件名，正文入库继续 |
| 批量删除中单篇清理失败 | 继续删除其他选中文档，失败项保留可重试记录 |
| 规划模型失败 | 使用原问题单查询 |
| 某个子查询失败 | 保留其他子查询结果；全空时重试原问题 |
| 原问题检索抛出基础设施异常 | 返回“检索服务暂不可用”，不伪装成“文献无证据” |
| rerank/压缩失败 | 各组件内部降级；编排层仍保留原始候选 |
| 证据仅涉及相邻主题 | 清空候选并固定拒答，不附加误导性参考来源 |
